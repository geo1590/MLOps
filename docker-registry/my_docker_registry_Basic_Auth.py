...
It uses the Basic Auth insecure method.


NOTES
==============
# For Basic Auth using ngrok, ngrok using the permanent public URL (ngrok's Domains) requires a connection that uses TLS. 
# Therefore, using the ngrok permanent public URL requires using the docker container registry in TLS HTTPS secure mode.
# But, using ngrok temporary public URL (ngrok's Endpoints) will work with Basic Auth method.
...
def do_install_docker_registry_basic_auth():
    ...
    if do_configs:
        ...
        commands = [
            ['id', 'use_same_session'],
            ['mkdir -p /registry/htpasswd', 'use_same_session'],
            ['cd /registry/htpasswd', 'use_same_session'],
            ['pwd', 'use_same_session'],
            # [f'sudo echo "{user_password}" | sudo htpasswd -i -c /registry/htpasswd/htpasswd {user_name}', 'use_same_session'],
            ['docker run -d -p 5000:5000 --restart=always --name registry \
                 -v /registry/htpasswd:/auth \
            ...
            ["kill -9 `ps -ef | egrep ngrok | egrep -v 'grep' | awk '{print $2}'`", 'use_same_session'],
            [f'nohup ngrok http 5000 &', 'use_same_session'],
    ...
    if do_local_tests:
        commands = [
            [f'docker login http://localhost:5000 -u {user_name} -p {user_password}', 'check_returncode'],
        ]
...
# user_password = input("Enter a password for '{user_name}': ")
